...
// da mesma maneira. Nunca aumenta um vídeo que já seja mais pequeno.
function targetSize(meta, shortSide) {
  if (shortSide === 'source') return {};

  const target = Number(shortSide);
  const current = Math.min(meta.width, meta.height);
  ...
  return meta.height > meta.width
    ? { width: even(meta.width * ratio), height: even(meta.height * ratio) }
    : { width: even(meta.width * ratio), height: even(meta.height * ratio) };
}
