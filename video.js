...
async function encodeAttempt(file, meta, settings, bitrateBudget, onProgress) {
...
export async function compressVideo(file, meta, settings, { onLoadProgress, onProgress }) {
  ...
  let result = await encodeAttempt(file, meta, settings, initialBudget, onProgress);
  ...
  if (result.size >= file.size && sourceBitrate) {
    for (const budgetFactor of [0.5, 0.3, 0.15]) {
      const candidate = await encodeAttempt(
        file,
        meta,
        settings,
